...
def list_finance(request: Request):
    with Session(engine) as session:
        ...
        return templates.TemplateResponse("finance.html", {"request": request, "payables": payables, "total_open": total_open, "config": get_config(session)})
...
def toggle_payable_status(pid: int, background_tasks: BackgroundTasks):
    with Session(engine) as session:
        ...
        return f"""<button hx-post="/finance/{pid}/toggle_status" hx-swap="outerHTML" class="{color} px-3 py-1 rounded-full text-xs font-bold border hover:scale-105 transition">{p.status}</button>"""
